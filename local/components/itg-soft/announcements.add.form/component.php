...
$arParams['ORGANIZATION_PROPS'] = trim($arParams['ORGANIZATION_PROPS']);
...
if ($bAllowAccess)
{
	...
	$arResult["PROPERTY_LIST_FULL"] = array(
		"ACTIVE" => array(
			...
			"ENUM" => array(
				'Y' => array(
					"DEF" => 'Y',
					"VALUE" => GetMessage('IBLOCK_ADD_ACTIVE_VALUE_YES'),
				),
				'N' => array(
					"DEF" => '',
					"VALUE" => GetMessage('IBLOCK_ADD_ACTIVE_VALUE_NO'),
				)
			)
		),
	...
	if(empty($arParams['PROPERTY_CODES'])){
		$arParams['PROPERTY_CODES'] = array_keys($arResult["PROPERTY_LIST_FULL"]);
	}
	...
	if ($arParams["ELEMENT_ASSOC"] == "PROPERTY_ID" && strlen($arParams["ELEMENT_ASSOC_PROPERTY"]) && strlen($arParams['ORGANIZATION_PROPS']))
	{
		if ($USER->GetID()){
			$userData = CUser::GetById($USER->GetId())->fetch();

			if(!empty($userData[$arParams['ORGANIZATION_PROPS']])){
				$arFilter["PROPERTY_".$arParams["ELEMENT_ASSOC_PROPERTY"]] = $userData[$arParams['ORGANIZATION_PROPS']];
			}else {
...
if ($bAllowAccess)
{
	...
	if (check_bitrix_sessid() && (!empty($_REQUEST["iblock_submit"]) || !empty($_REQUEST["iblock_apply"])))
	{
		...
		foreach ($arParams["PROPERTY_CODES"]  as $i => $propertyID)
		{
			...
			if (intval($propertyID) > 0)
			{
					...
					else
					{
						if ($arResult["PROPERTY_LIST_FULL"][$propertyID]["PROPERTY_TYPE"] != "L")
							if($arResult["PROPERTY_LIST_FULL"][$propertyID]['USER_TYPE'] == 'HTML'){
								$arUpdatePropertyValues[$propertyID]['VALUE'] = $arPropertyValue[0];
							}else{
				...
				else
				{
					...
					if(!empty($arPropertyValue)){
						...
						$tmpFiles = !empty($_REQUEST['PROPERTY_FILE_' . $propertyID]) ? $_REQUEST['PROPERTY_FILE_' . $propertyID] : array();
						...
						foreach ($tmpFiles as $fileId) {
							$filePath = CFile::GetPath(intval($fileId));

							if(!empty($filePath)){
								$fileArray = CFile::MakeFileArray($filePath);
								$arUpdatePropertyValues[$propertyID][] = $fileArray;
							}
						}
					}
			...
			if($arResult["PROPERTY_LIST_FULL"][$propertyID]['CODE'] == 'RENTAL_COMPANY' && !empty($arParams['ORGANIZATION_PROPS'])){
				$userData = CUser::GetByID($USER->GetId())->fetch();
				$arUpdatePropertyValues[$propertyID] = array($userData[$arParams['ORGANIZATION_PROPS']]);
			}
		}
		...
		if (empty($arResult["ERRORS"]))
		{
			...
			if(!empty($arProperties['ACTIVE']) && $arProperties['ACTIVE'] == 'Y'){
		...
		if (empty($arResult["ERRORS"]))
		{
			...
			if($arParams["ID"]){
				$sRedirectUrl .= (strpos($sRedirectUrl, "?") === false ? "?" : "&") . "CODE=" . $arParams["ID"];
			}
