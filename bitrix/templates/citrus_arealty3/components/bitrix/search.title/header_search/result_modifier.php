...
foreach($arResult["SEARCH"] as $i=>&$arItem)
{
	switch($arItem["MODULE_ID"])
	{
		case "iblock":
            ...
            $res = CIBlockElement::GetList(array(), array("ID" => $arItem["ITEM_ID"]), false, false, $arSelect);

            if($ob = $res->GetNextElement()) {
                $arResItem =  $ob->GetFields();
                $arResItem["PROPERTIES"] = $ob->GetProperties();

                $arItem["PREVIEW_PICTURE"] = $arResItem["PREVIEW_PICTURE"];
                $arItem["DETAIL_PICTURE"] = $arResItem["DETAIL_PICTURE"];
                $arItem["ADDRESS"] = (string)\Citrus\Arealty\Object\Address::createFromFields($arResItem);
                $arItem["COST"] = $arResItem["PROPERTIES"]["cost"]["VALUE"];
                if($arItem["COST"]) {
                    $arItem["COST"] = number_format($arItem["COST"], 0, ',', ' ').' <span class="currency-icon"><span class="rub"></span></span>';
                }

                if ($arParams["SHOW_PREVIEW"] == "Y"){
                    $arFile = false;
                    if ($arItem["PREVIEW_PICTURE"] > 0)
                        $arFile = CFile::ResizeImageGet($arItem["PREVIEW_PICTURE"], array("width"=>$PREVIEW_WIDTH, "height"=>$PREVIEW_HEIGHT), BX_RESIZE_IMAGE_EXACT, true);
                    elseif ($arItem["DETAIL_PICTURE"] > 0)
                        $arFile = CFile::ResizeImageGet($arItem["DETAIL_PICTURE"], array("width"=>$PREVIEW_WIDTH, "height"=>$PREVIEW_HEIGHT), BX_RESIZE_IMAGE_EXACT, true);

                    if($arFile) $arItem["PICTURE"] = $arFile["src"];
                }
