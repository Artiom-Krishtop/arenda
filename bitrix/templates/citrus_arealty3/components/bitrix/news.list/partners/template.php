	...
	<?foreach($arResult["ITEMS"] as $arItem):?>
		...
		<div class="b-partners-list-item" id="<?=$this->GetEditAreaId($arItem['ID']);?>">
			...
			if ($arPicture):
				...
				<?php if ($urlIsNotEmpty = (trim($arItem["DISPLAY_PROPERTIES"]["url"]["VALUE"]) != "")) {
					?><a href="<?= $arItem["DISPLAY_PROPERTIES"]["url"]["VALUE"] ?>" target="_blank" rel="nofollow"><?php
				} ?>
				<img class="b-partners-preview-logo b-partners-list-preview-picture" border="0" src="<?=$arSmallPicture["src"]?>" width="<?=$arSmallPicture["width"]?>" height="<?=$arSmallPicture["height"]?>" alt="<?=$arItem["NAME"]?>" title="<?=$arItem["NAME"]?>" />
				<?php if ($urlIsNotEmpty) {
					?></a><?php
				} ?>
			<?endif?>
