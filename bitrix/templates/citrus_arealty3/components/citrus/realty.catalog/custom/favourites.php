...
<?if(\ITG\Custom\Favourites::getCount() > 0 || !empty($favouritesFromSharedUrl)):
	//default sort
	$arSortFields = array(
...
<?php if (\ITG\Custom\Favourites::getCount() > 0 || !empty($favouritesFromSharedUrl))
{
	?>
	<div class="favorite-button-group">
		...
		<div class="share-component <?= empty($arrFavouritesFilter["ID"]) ? 'hidden' : ''?>">
			...
			<div 
				...
				data-title="<?= GetMessage("CITRUS_REALTY_FAV_SHARE_TITLE")?>"
				data-url="<?= $arResult["URL_FOR_SHARE"]?>"
			></div>
		</div>
	</div>
	<?php
...
<script>
	BX.message({CITRUS_AREALTY_PDF_SEND_SITE_DIR: '<?= CUtil::JSEscape('/printpdf/') ?>',});
</script>
